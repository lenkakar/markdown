...
- ORDER BY - это оператор в языке запросов SQL, который позволяет сортировать полученные строки по возрастанию или убыванию.
```
...
- HAVING -  определяет, какие группы будут включены в выходной результат, то есть выполняет фильтрацию групп.  Условия в HAVING проверяются после группировки, поэтому они задаются после предложения GROUP BY.
...
- это запрос, который содержится в предложении WHERE или HAVING другого оператора SQL.
